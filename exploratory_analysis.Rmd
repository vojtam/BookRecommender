...
plot_hist <- data_main |>
  ...
  geom_histogram(color = "black", fill = "palegreen2", bins = 50) +
...
plot_hist<- data_desc |> ggplot(aes(x = description_length)) +
  geom_histogram(color = "black", fill = "plum1") +
  labs(title = "Length of Description Histogram", x = "Length of Description (in Words)") +
  xlim(0, 600) +
  theme(plot.title = element_text(hjust=0.5))

boxplot <- data_desc |> ggplot(aes(y = description_length, x = "")) + 
  geom_boxplot(color = "black", alpha = 0.3, fill = "plum1", outlier.shape = NA) + 
  geom_jitter(pch = 21, size = 2.5, width = 0, height = 0.1) + 
...
plot_hist <- data_main |> 
  ...
  geom_histogram(color = "black", fill = "salmon1", bins = 60) +
...
## Users Reviews
```{r}
data_reviews <- read.csv2(file = "data/reviews_tab_filtered.csv", sep=",")
...
plot_hist <- data_reviews |> 
  ...
  geom_histogram(fill = "orange1", color = "black", bins=40)+
  labs(title = "Average Rating of One User Histogram", x = "Average Rating of One User")

boxplot <- data_reviews |> 
  group_by(user_id) |> 
  dplyr::summarize(mean_user_rating = mean(rating)) |>  
  ggplot(aes(y = mean_user_rating, x = "")) + 
  geom_boxplot(color = "black", alpha = 0.3, fill = "orange1", outlier.shape = NA) + 
  geom_jitter(pch = 21, size = 2.5, width = 0, height = 0.1) + 
  labs(title = "Average Rating of One User Boxplot", x = "Average Rating of One User", y ="Stars")
