...
str(books_ratings)
```
We will only retain users and books with 10 and more ratings
```{r}
...
Saving the model
```{r}
saveRDS(book_reco_SVDF, file = "svdf.rds")
```
...
Saving the model
```{r}
saveRDS(book_reco_UBCF, file = "ubcf.rds")
```
## UBCF Normalized
```{r}
...
Saving the model
```{r}
saveRDS(book_reco_UBCF_centered, file = "ubcf_normalized.rds")
```
```{r}
